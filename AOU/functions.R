...
write_RDATA_to_bucket <- function(object_list, destination_filename) {
  if (!grepl("RData$", destination_filename, ignore.case = T)) {
    stop("Must write to an .RData file")
  }
...
load_RDATA_from_bucket <- function(filename, skip_copy = T) {
  if (!grepl("RData$", filename, ignore.case = T)) {
    stop("Must provide an .RData file")
  }
...
slip_condition_first <- function(condition_name) {
  ...
  dat_slip <- CCI_first %>%
    ...
    mutate(
      year = decimal_date(first_occurrence) - decimal_date(ogn),
      condition_status = 1
    )
...
extract_condition <- function(where_ICD9_like, where_ICD10_like, name = "condition") {
  ...
  names(dataset_first)[2:5] <- c(
    paste0("first_", name, "_date"), paste0(name, "_concept_source_code"),
...
extract_procedure <- function(code_in, name = "procedure") {
  ...
  dataset_df <- read_bq_export_from_workspace_bucket(procedure_path) %>%
    mutate(procedure_datetime = as.Date(procedure_datetime)) %>%
    distinct() %>%
...
extract_medication <- function(concept_ids, name) {
  ...
  message(str_glue(
    "The data will be written to {med_path}. Use this path when reading ",
...
pull_lab <- function(concept_ids_string = "xxxx", name = "biomarker", read_existing = FALSE, remove_NA = TRUE) {
  ...
  if (name == "biomarker") {
    stop("Provide a name for the biomarker")
  }
  ...
  if (!read_existing) {
    ...
    tic(paste("Reading existing pulled data for ", name))
  }

  read_bq_export_from_workspace_bucket <- function(export_path) {
    col_types <- cols(
      standard_concept_name = col_character(), unit_concept_name = col_character(),
...
pull_observation_with_value <- function(concept_ids_string = "xxxx", name = "observation", read_existing = FALSE) {
  # example string: "37047736, 4146380"
  if (name == "observation" | is.null(name) | is.na(name)) {
    stop("Provide a name for the observation")
  }
...
pull_observation <- function(concept_ids_string = "xxxx", name = "observation", read_only = FALSE) {
  ...
  read_bq_export_from_workspace_bucket <- function(export_path) {
    col_types <- cols(
      standard_concept_name = col_character(), observation_type_concept_name = col_character(),
...
dedup_median <- function(data) {
  ...
  if (nrow(m10m_diffconcept) > 0) {
    ...
    suppressMessages(
      m10m_summ1 <- m10m_diffconcept %>%
        ...
        mutate(
          unit_concept_name = ifelse(unit_concept_name == "", NA, unit_concept_name),
  ...
  suppressMessages(
    alldata <- full_join(m1, m10_all) %>%
      select(any_of(c(
        "person_id", "measurement_date", "value_as_number", "unit_concept_name", "standard_concept_name",
...
pull_condition <- function(concept_ids_string = "xxxx", name = "condition", read_only = FALSE) {
  if (name == "condition" | is.null(name) | is.na(name)) {
    stop("Provide a name for the condition")
  }
  ...
  if (!read_only) {
    ...
    tic(paste0("Reading in existing pull for ", name))
  }
...
pull_procedure <- function(concept_ids_string = "xxxx", name = "procedure", require_standard = TRUE) {
  if (name == "procedure") {
    stop("Provide a name for the procedure")
  }
  ...
  read_bq_export_from_workspace_bucket <- function(export_path) {
    col_types <- cols(
      standard_concept_name = col_character(), source_concept_code = col_character(),
...
getUniqueDescendants <- function(concept_ids_string) {
  ...
  rt <- tmp %>%
    filter(concept_id != descendant_concept_id) %>%
...
code_sequence <- function(x, min_sequence_days, new_sequence_days, min_n_codes, prefix = "seq") {
  seqs <- x |>
    ...
    mutate(
      days_since_last_dt = as.numeric(event_date - lag(event_date)),
...
align_date_dm <- function(data, marker_names = "value_as_number", date_name = "measurement_date", years_prior = 2) {
  data <- data %>%
    dplyr::rename(marker_date = date_name) %>%
...
med.gap <- function(data, min_gap_days = 30, years_prior = 0, mult.by.refills = TRUE) {
  meds_time <- data %>%
    right_join_quiet(DMcohort %>% dplyr::select(person_id, DmDx_first, DeathDateTime)) %>%
    mutate(
      tstart = drug_exposure_start_datetime - DmDx_first,
...
# Quantiles
q25 <- function(x) {
  signif(quantile(x, 0.25, na.rm = T), 3)
}
q75 <- function(x) {
  signif(quantile(x, 0.75, na.rm = T), 3)
}
IQR <- function(x) {
  paste0(signif(median(x, na.rm = T), 3), " (", q25(x), " - ", q75(x), ")")
}
...
yr_round <- function(x) {
  x %>%
    ...
    mutate(
      yrs = measurement_date - DmDx_first,
...
man_cal_quantiles <- function(pred, truth, htime, conf_level = 0.95, ntile = 4) {
  if (max(truth$CR_tte_landmark) >= htime) {
    if (!is.null(pred) & length(pred) > 0) {
      ...
      binned <- data.frame(Note = "Predictions not provided")
    }
  ...
  for (htime in htimes) {
    ...
    for (i in 1:n_bootstrap) {
      ...
      if (!is.null(troc_boot$AUC_1)) {
        aucdat_boot <- cbind(
          "AUC_1" = troc_boot$AUC_1[2],
          "AUC_2" = troc_boot$AUC_2[2]
        ) %>%
          data.frame() %>%
          mutate(boot_n = i)
      } else {
        aucdat_boot <- cbind(
          "AUC_1" = troc_boot$AUC[2],
          "AUC_2" = troc_boot$AUC[2]
        ) %>%
          data.frame() %>%
          mutate(boot_n = i)
      }
...
subset_group <- function(data, subgroup, EGFRdat) {
  if (subgroup == "Black/African American") {
    ...
    (stop(paste("subgroup ", subgroup, " not valid")))
  }
}
...
calc_RECODe_riskscore <- function(landmark_data) {
  coefs <- data.frame(
    ...
    coef = c(
      -0.01938, -0.01129, 0.08812, 0.23380, 0.14830, 0.00303, -0.02164,
      -0.07952, -0.12560, 0.03199, 0.13690, -0.00111, 0.00629, 0.86090, 0.00036
    )
  )
  data <- landmark_data %>%
    mutate(
      EthnicityHISPANIC.OR.LATINO = as.numeric(Ethnicity == "Hispanic or Latino"),
    ...
    mutate(
      UACR = coalesce(UACR, 7.5),
      bp_lowering_drugs = ACEInhibitors | AngiotensinIIRB | Diuretics | BetaBlockers | CChannelBlockers,
      oraldmdrug = DPP4 | SGLT2 | Sulfonylurea | Thiazoladinedione | Other_oraldm
    ) # metformin not included

  data_long <- data %>%
    select(person_id, all_of(unique(coefs$predictor))) %>%
...
predict_prob_calibrated_cox <- function(ftime, fstatus, horizons, betax) {
  ...
  recalibLP_cox <- coxph(Surv(ftime, event_ESRD) ~ betax,
    data = tmpdatIN,
    y = T, x = T
  )
